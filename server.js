...
function applyFFData(runners, ffData) {
  ...
  return runners.map(function(r) {
    ...
    const ffR  = ffData.runners.find(function(f) {
      const fn = (f.name || '').toLowerCase().replace(/[^a-z]/g, '');
      return fn === name || fn.substring(0,8) === name.substring(0,8);
    });
    if (!ffR || ffR.scratched) return r;
    const dec = ffR.decorators || [];
    const comboD = dec.find(d => d.type === 'jockey_trainer_combo');
    return Object.assign({}, r, {
      ffRunningStyle:      (ffR.speedMap || {}).runningStyle || null,
      ffWithinOptimal:     (ffR.raceClassFit || {}).withinOptimalRange,
      ffClassFit:          (ffR.raceClassFit || {}).assessment || '',
      ffRatingTrend:       (ffR.classProfile || {}).trend || '',
      ffLastStartWinner:   dec.some(d => d.type === 'last_start_winner'),
      ffWeightDrop:        dec.some(d => d.type === 'weight_drop'),
      ffJockeyTrainerCombo: !!comboD,
      ffComboWinRate:      comboD ? parseFloat(((comboD.detail||'0').split('%')[0].split('(')[1])||0)/100 : null,
      ffGearChange:        !!ffR.gearChange,
    });
  });
}

function calcFFBonus(runner, distance) {
  let bonus = 0;
  if (runner.ffRunningStyle === 'P') bonus += distance <= 1200 ? 8 : distance <= 1600 ? 4 : 2;
  else if (runner.ffRunningStyle === 'B') bonus += distance <= 1200 ? -6 : distance <= 1600 ? -2 : 2;
  ...
  if (runner.ffGearChange) bonus -= 2;
...
async function runScanner() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      // Fetch FormFav enrichment
      let ffData = null;
      if (FF_KEY) {
        ffData = await fetchFFRace(venue, raceNum, dateStr);
        await sleep(400);
      }
      ...
      // Apply FormFav enrichment
      if (ffData) runners = applyFFData(runners, ffData);

      // Score including FF bonus
      runners = runners.map(r => ({
        ...r,
        algoScore: scoreRunner(r, raceConditions) + calcFFBonus(r, distance)
